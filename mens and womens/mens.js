...
// console.log(mensData[0].price)
let status = JSON.parse(localStorage.getItem("status")) || [];
let container = document.querySelector("#mens");
...
function display(mensData) {
  container.innerText = "";
  mensData.forEach(function (e, ind) {
    ...
    button.addEventListener("click", function () {
      addToCart(e, ind, button);
    });

    Product.append(img, name, price, button);
    container.append(Product);
  });
}
let filter1 = document.querySelector("#sortNames");
...
function checkCart(ele) {
  cartLS = JSON.parse(localStorage.getItem("cart-page")) || [];
  for (let i = 0; i < cartLS.length; i++) {
    if (cartLS[i].name == ele.name) {
      return true;
    }
  }
  return false;
...
function addToCart(ele, ind, btn) {
  if (!checkCart(ele)) {
    ele["count"] = 1;
    cartLS.push(ele);
